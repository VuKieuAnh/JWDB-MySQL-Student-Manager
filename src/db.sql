...
select name_classes, count(name_classes) 'Số lượng học viên các lớp'
from classes
         join students s on classes.id_classes = s.id_classes
group by name_classes;
# Thống kê số lượng học viên các tỉnh
select name_address, count(name_address) 'Số lượng học viên các tỉnh'
from address
         join students s on address.id_address = s.id_address
group by name_address;
# Tính điểm trung bình các khóa học
select name_course, avg(point) 'Điểm trung bình'
from point
...
from point
         join course c on point.id_course = c.id_course
group by name_course having  avg_point >= ALL
